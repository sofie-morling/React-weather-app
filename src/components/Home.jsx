...
export const Home = () => {
  ...
  // Geolocation finding your current location
  const findUser = (evt) => {
    if (evt) {
  ...
  useEffect(() => {
    fetch(
      `https://hendrik-cors-proxy.herokuapp.com/api.openweathermap.org/data/2.5/weather?lat=${position.latitude}&lon=${position.longitude}&units=${unit}&appid=${api.key}`
    )
      .then((res) => res.json())
      .then((result) => {
        setWeather(result);
  ...
  // Convertion from unixtimestamp to human readable
